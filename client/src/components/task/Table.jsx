...
const Table = ({ tasks }) => {
  ...
  const [tasksList, setTasks] = useState(tasks); // Track tasks in state
  ...
  const deleteHandler = async () => {
    try {
      await axios.put(`/api/task/${selected}`, { isTrashed: true }); // Update task to mark as trashed
  ...
  const TableHeader = () => (
    <thead className="w-full border-b border-gray-300">
      <tr className="w-full text-black text-left">
        ...
        {isAdmin && <th className="py-2 text-center">Action</th>}
      </tr>
    </thead>
  );
